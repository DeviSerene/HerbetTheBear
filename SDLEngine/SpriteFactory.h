...
public:
	...
	~SpriteFactory();
	void Draw(std::string _name, SDL_Rect _position, bool _flipHorizontal = false, float offsetX = 0, float offsetY = 0);
	void Draw(std::string _name, SDL_Rect _position, SDL_Rect& _cellRect, bool _flipHorizontal = false, float offsetX = 0, float offsetY = 0); //two Draws, for static/animation
